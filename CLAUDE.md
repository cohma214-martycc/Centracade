...
- **TAZO TYCOON** (`par:4500` sim-derived, PALS[7], press+release, no drag) — 4×4 directional-merge; tiers 0 slate → 1 kraft → 2 holo cyan → 3 gold → 4 mega slammer white (`TAZO_COL` ramp, recoloured 6.2 for separation — all `// PROVISIONAL`; + centred `drawMark`). Merge once per tile per move; slide tween + double-strike bounce + `clack()`. **Mega slammer detonates orthogonal CLUTTER only (tier ≤ `SLAM_MAXTIER 1`) and is consumed — full-radius clearing makes the board a perpetual-motion machine (sim-proven); do not "restore" it.** Escalating spawn pressure is first-class: count `min(12, 1+floor(moves/SPAWN_K 6))` (moves-based, skill-proof) + floor-tier rises with score (`TIER_BANDS`, tier-0 gone by 360). Loss = board-lock, ONE terminal state (owner-approved D14 exception). Time caps forbidden (D3). **Owner: plays EASY, but the 6.2 sim proved the sanctioned spawn levers can't bound a skilled run (D27) — difficulty deferred to the 6.5 par + daily-length call. Colour separation shipped (6.2): tiers recoloured slate/kraft/cyan/gold/white, all `// PROVISIONAL`, owner to eyeball on-device.**
...
- **Difficulty — RESOLVED: deferred to 6.5 (D27, owner call 2026-07-20).** The 6.2 plan was `SPAWN_K 6→4/5` + lower `TIER_BANDS`, sim-validated to ~2–2.5 min median. A faithful headless replica (3000 runs × greedy/survival/random policies, anchored to the doc's current greedy ≈3.3 min) showed those levers are **nearly inert for skilled play**: `SPAWN_K 6→4` moves the greedy median only 3.30→3.05 min, lowering the bands *raises* it slightly (higher-tier spawns pay more), and lowering `SPAWN_CAP` makes runs *longer*. Worse, the **~12 min p90 tail is immovable across every config** — a good player clears space faster than spawns fill it, so spawn pressure never bounds the run. Even aggressive off-spec configs (spawn base 2–3, K2) only reach ~2.8 min greedy / ~3.5 min survival and still don't touch p90. **No spawn-tuning ships** — the owner chose to accept run length now and handle it holistically at the 6.5 par retune + daily-length decision, with real `arc_stats` data. Do not re-attempt `SPAWN_K`/`TIER_BANDS` tuning (D27). (Sim: reproducible from the mechanics in this entry.)
- **Colour separation — ✅ SHIPPED (`// PROVISIONAL`, owner to eyeball on-device).** `TAZO_COL` recoloured: tier-0 plastic → **slate grey** (`#9aa7b8` edge; was blue `#6a8cff`, ΔE only 11.6 from the board ring — the clash), tier-1 cardboard → **kraft brown** (`#b06a34`; was amber `#e0a84a`, ΔE 22 from tier-3 gold), tier-2 cyan / tier-3 gold / tier-4 white unchanged. Verified with CIELAB ΔE: every tier-pair now ≥33.6 (previously-confusable pairs → t0↔board-ring 66.7, t1↔t3 49.4) and every tier ≥59.6 from the PALS[7] navy field. TAZO menu-icon fan updated to the new distinct tiers (brown/gold/cyan); the two retired hexes are gone from the game.
...
| D27 | **TAZO run-length is a 6.5 par + daily-length problem, not a spawn-tuning one** (owner call, 2026-07-20). A faithful Unit-6.2 sim proved the sanctioned `SPAWN_K`/`TIER_BANDS`/`SPAWN_CAP` levers barely move the median and can't touch the ~12 min p90 tail (a skilled player out-clears spawn pressure). No spawn-tuning ships; difficulty is deferred to the 6.5 par retune + daily-length decision with real `arc_stats` data. Do not re-attempt spawn-lever tuning. The 6.2 colour separation shipped independently. |
...
- **TAZO tier hexes** — slate/kraft recolour for tiers 0–1 SHIPPED (6.2, `// PROVISIONAL`, ΔE-validated); owner to eyeball on-device and confirm the exact hexes (and whether the guessed pairs — cardboard↔gold, plastic↔board-blue — were the real confusables).
- ~~**TAZO difficulty (6.2)**~~ — RESOLVED 2026-07-20 (D27): the sim proved the sanctioned levers can't bound a skilled run, so difficulty is deferred to the 6.5 par + daily-length call with real telemetry. No spawn-tuning ships.
