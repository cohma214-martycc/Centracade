...
- **ROUND theme D15 sign-off.** Mowing the lawns is brand-free (hose, sprinkler, brick — no named product) and shipped on the RECALL precedent. Sign-off owed to close D43 for ROUND, along with its `🌱` / `the lawns` receipt line.
...
- **CHUTE theme D15 sign-off.** The paper run names no masthead — the three bags are a colour, a fold and a size, and the copy says `SORT BY MASTHEAD / FOLD / SIZE` without naming a publication. Brand-free as built, shipped on the RECALL precedent; sign-off owed to close D43 for CHUTE, along with its `📰` / `paper run` receipt line.
...
- **LOCK theme D15 sign-off.** The diary padlock is brand-free (no named product) and shipped on that basis, per the precedent set when RECALL's dial-up main was cleared. Owner sign-off still owed to close D43 for LOCK.
- **LOCK share icon/label** — shipped as `🔒` / `diary lock`, working values pending the same D34/D15 pass as ROUND and CHUTE.
...
| **LOCK** | **🔒** | **diary lock** | — shipped 2026-08-02; D15 sign-off owed |
| **ROUND** | **🌱** | **the lawns** | — shipped 2026-08-02; D15 sign-off owed |
| **CHUTE** | **📰** | **paper run** | — shipped 2026-08-02; D15 sign-off owed |
