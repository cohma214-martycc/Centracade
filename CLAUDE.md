...
**Phase 5 is underway and is the largest change since Phase 2**: CHAIN is **pruned (Unit 5.0, shipped)**, **GUNGE has shipped (Unit 5.2)**, **HOWLER has shipped (Unit 4)**, **TAZO TYCOON has shipped (Unit 5)**, and two deliberately *different-feeling* games still join — DAIRY WARMER, WEAVER. The existing roster was stylistically narrow (tap-timing and tap-target). Phase 5 adds swipe-analog (HOWLER — the first swipe game in the hub), grid-rotation (GUNGE — the first puzzle), directional-merge (TAZO — the first 2048-style game), spatial-packing, and path-drawing. Roster goes **7 → 6 → 11** — currently at **9** (GUNGE + HOWLER + TAZO landed; two new games pending).
...
Phase 5 palette entries (added Unit 2, unreferenced until each game lands): HOWLER reuses `0` (**now referenced** — `PALMAP.howler=0`, menu-icon + card lookups); **`6` GUNGE** slime lime (**now referenced** — `PALMAP.gunge=6`, menu-icon + card lookups), **`7` TAZO** electric holo blue (**now referenced** — `PALMAP.tazo=7`, menu-icon + card lookups; also the `TAZO_COL` tier ramp derives from this family), **`8` DAIRY** warm caramel, **`9` WEAVER** navy + red-sock. `8`/`9` hues stay **tentative** — retune when the owning game is built. Drawing a surface directly (asphalt-court precedent) is still fine where a full palette isn't warranted.
...
**⬜ TODO — HOWLER revision pass (owner-requested; NOT yet in `index.html`):**

1. **Four-step distance ladder — `30 m → 60 m → 90 m → ULTIMATE DISTANCE`.** A homage to the 2003 Dan Carter & Steve Devine Mega Howler TV ad. Surface these as **named distance tiers** the player climbs (HUD label, stage/rank feel, escalating difficulty on each rung). **Keep it compatible with D14**: HOWLER stays **endless + 3 lives** — the four tiers are *named milestones on the continuous ramp*, not a finite 4-level game that ends at "ultimate distance." The current code ramps `centerR`/`rimR`/wind smoothly per scoring hit; this TODO reframes that ramp around the four named checkpoints (decide at build: relabel the existing stages onto the four tiers, or hold each tier for N hits before advancing, then let ULTIMATE DISTANCE ramp open-endedly). The ranks list may want to echo the ladder.
2. **Recolour the Mega Howler — purple body, green/teal fins.** Cosmetic change to `drawRocket()` **and** the `drawMenuIcon` howler branch (both currently ember/orange body from PALS[0] with `#e85d15` fins). **Reconcile with the palette maps**: HOWLER is `PALMAP.howler=0` (ember) in all three maps, so the *card/background/HUD glow* is ember while the *rocket* would go purple — decide whether that split is intended (rocket is a distinct object, like GUNGE's drawn surfaces) or whether HOWLER should move to a purple-ish palette entry (note PALS[2] purple is the reserved hub palette — do not repurpose it for a game; add/point at a new entry if a full repalette is wanted). Cheapest path: leave PALMAP at 0, just recolour the drawn rocket sprite + icon.
...
- **Roster**: slotted into `GAMES` **after CHATTER** (before SCRAMBLE) per §5.6; `PALMAP.tazo=7` in all three palette maps; drawn menu icon (fanned tazo stack). Menu count / emoji grid / `drawBars` all derive from `GAMES.length`. Daily is now **9 games**.
...
- **New `PALS` entries for Phase 5** — ✅ added (Unit 2), after the CHATTER explicit-cycle fix that made growth safe. Four entries (`6` GUNGE, `7` TAZO, `8` DAIRY, `9` WEAVER); **GUNGE ships on `6`**, **TAZO ships on `7`** (retune with the roster), `8`/`9` hues **remain open** — provisional, retune per game. Drawing surfaces directly (asphalt-court precedent) is still available where a palette isn't warranted.
