...
**Phases 1–5 are SHIPPED.** The roster is **14 games shipped** (STACK · SCAN · HOWLER · SWINGBALL · GUNGE · CHATTER · TAZO · SCRAMBLE · DAIRY · KNUCKLEBONES · WEAVER · **RECALL** · **LOCK** · **ROUND**) **targeting 15 gross** (D36), plus the once-a-day seeded **TUCK SHOP RUN** daily, which draws a day-seeded subset rather than the whole roster (D33; `DAILY_PICK` is **5** since RECALL landed, D37). The roster was reopened at D36 for **four** additions — RECALL ✅ · LOCK ✅ · ROUND ✅ · CHUTE (Phase 7.5) — after which it closes again pending prune calls (D26). Work now proceeds in this order:
...
**Pointer stream (D12, wired):** a game declares the stream by defining `press`. The router (`gameGesture` + `gameDown()`) then forwards down/move/up and never calls `tap()` during play; `ready`/`over` interactions still resolve through `tap()` **on release**. Works identically in free play and the gauntlet. `pointercancel` routes to `release(lastCoords)` — an abandoned drag can never hang a gesture. Consumers: HOWLER (press+drag+release, drag = aim arrow + power meter), TAZO and **ROUND** (press+release, no drag by design), WEAVER (full consumer — all three carry live state). GUNGE, DAIRY (D13) and **RECALL** + **LOCK** (7.5) are tap-only. Games without `press` keep zero-latency tap-on-pointerdown, untouched — so B6's release-latency cost does not apply to them. Keep `gameGesture` and `menuDrag` strictly separate.
...
- **Prefix-deterministic — RECALL, LOCK and ROUND (7.5)**, the strongest form in the codebase and worth understanding before copying it. The stream is consumed as **one growing prefix**, at a rate that is a pure function of *how far the player got*, never of *how they played*. Skill changes how far down the sequence you get, never what it is.
  ...
  - ⚠ All three are fragile the same way: a per-round decoy/shuffle (RECALL), a seeded hint/diary pick (LOCK), or a mid-course spawn (ROUND) would make consumption depend on the player again, which is B1 in a different costume. **Draws per unit must stay a function of the index, not the play.**
...
2. **§2 keys table** — ✅ `arc_recall_best`, ✅ `arc_lock_best`, ✅ `arc_round_best`; add `arc_chute_best` when it ships. No legacy-key inheritance for any of them (D1 precedent).
3. **§3 determinism model** — ✅ RECALL and ✅ LOCK both added to the *prefix-deterministic* bullet (draws per unit are a function of the index, not the play; stronger than fixed-count and documented as such). ✅ ROUND joined them (whole course dealt per stage, 3 draws/row). Add CHUTE the same way. None of the four may join the *inherently divergent* list.
4. **§3 pointer-stream note** — ✅ RECALL and ✅ LOCK logged tap-only; ✅ ROUND logged as a `press`+`release` consumer (no `drag`), TAZO-style. CHUTE joins it there.
5. **§4** — ✅ RECALL, ✅ LOCK, ✅ ROUND. One as-shipped entry per game, written from the code, not from this brief.
6. **`GAMES` array** — ⚠ **pacing is moot, and the brief was wrong about this.** Since D33 the daily is a day-seeded *shuffle* of a subset, so this array no longer paces a run — it paces the **menu**. RECALL was therefore **appended**, not slotted mid-array: at 11 games the last page held 3 cards and a gap, so appending filled the gap and moved no existing card off its page or position. ✅ LOCK and ✅ ROUND were appended the same way and now share page 4. Do the same for CHUTE unless there's a menu-layout reason not to.
7. **`PALMAP`** — ✅ `recall:10` on a new `PALS[10]` (dial-up magenta, ~310°, the largest unused hue gap — reuse would have put a 12th card in an already-crowded band next to WEAVER's red). Appending `PALS` is safe by construction (§3). ✅ `lock:4` **reuses** SCRAMBLE's party pink — after RECALL every remaining hue gap sits inside the crowded green band, and reuse is precedented (HOWLER shares `0`, CHATTER shares `3`). `11` ROUND lawn green (~120°) — added rather than reused because the one gap still wider than 50° (GUNGE's yellow-lime 90° → SWINGBALL's blue-mint 148°) happens to be exactly the hue a mowing game wants. ✅ `round:11` on a **new** `PALS[11]` lawn green — the last gap wider than 50° sat exactly where a mowing game wants to be, so this one earned a row where LOCK didn't. One entry still to add; a missing one must still degrade to `PALS[2]`, never throw.
...
- **D15 clearance for the Phase-7.5 themes (D43).** ✅ **RECALL's dial-up main is cleared and shipped** (owner, 2026-08-02) — the first of the four. Still owed: the remaining **mains** (diary and lawns both shipped brand-free on that precedent and need only sign-off; the paper run is unbuilt — brand-free as specified, so each can be built and cleared as its game lands, RECALL's precedent), plus **all four alternates**, which block 7.2 specifically: Eagle Boys and the Pizza Hut `0800 83 83 83` number (RECALL alternate), Sky (LOCK alternate), Video Ezy (CHUTE alternate), school cross country (ROUND alternate — brand-free but unconfirmed), and any named masthead if CHUTE's paper run stops being generic.
- **Share icons and share labels for the four new games** (D34 receipt lines, D15 clearance). ✅ RECALL shipped as `📞` / `dial-up`, cleared with its theme. LOCK (`🔒` / `diary lock`) and ROUND (`🌱` / `the lawns`) shipped as working values; CHUTE still owed. All three need the same D15 pass.
...
- **Menu paging at 15 cards.** ✅ 5 pages since LOCK; ROUND joined it on page 4, and CHUTE fills that page to three. Dots/arrows screenshot-checked at 5 pages and read cleanly. No further paging work expected — 15 cards still fits 5 pages.
