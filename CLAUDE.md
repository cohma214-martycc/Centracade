...
- **Difficulty (direction known) — ⚠ BLOCKED, sim disproved the sanctioned levers.** The 6.2 plan was `SPAWN_K 6→4/5` + lower `TIER_BANDS`, sim-validated to ~2–2.5 min median. A faithful headless replica (3000 runs × greedy/survival/random policies, anchored to the doc's current greedy ≈3.3 min) shows those levers are **nearly inert for skilled play**: `SPAWN_K 6→4` moves the greedy median only 3.30→3.05 min, lowering the bands *raises* it slightly (higher-tier spawns pay more), and lowering `SPAWN_CAP` makes runs *longer*. Worse, the **~12 min p90 tail is immovable across every config** — a good player clears space faster than spawns fill it, so spawn pressure never bounds the run. Even aggressive off-spec configs (spawn base 2–3, K2) only reach ~2.8 min greedy / ~3.5 min survival and still don't touch p90. **Shipping a `SPAWN_K`/`TIER_BANDS` number here would guess against the data.** Real fixes need either a mechanic change beyond the sanctioned knobs (steeper spawn base — changes early feel), accepting the length and handling it via the 6.5 par + daily-length call, or a prune (D26). Owner decision flagged in §7. (Sim: scratchpad, reproducible.)
...
- **TAZO difficulty (6.2)** — owner must choose a path: the sim shows the sanctioned `SPAWN_K`/`TIER_BANDS`/`SPAWN_CAP` levers can't reach the ~2–2.5 min target or fix the ~12 min p90 tail. Options: (a) accept run length, retune only via the 6.5 par + daily-length call; (b) a stronger spawn-base change (off the sanctioned knobs — changes early feel); (c) prune TAZO (D26). Not shipped pending this call.
