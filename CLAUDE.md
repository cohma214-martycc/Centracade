...
**Phase 5 is underway and is the largest change since Phase 2**: CHAIN is **pruned (Unit 5.0, shipped)**, **GUNGE has shipped (Unit 5.2)**, **HOWLER has shipped (Unit 4)**, **TAZO TYCOON has shipped (Unit 5)**, **DAIRY WARMER has shipped (Unit 6)**, and one deliberately *different-feeling* game still joins — WEAVER. The existing roster was stylistically narrow (tap-timing and tap-target). Phase 5 adds swipe-analog (HOWLER — the first swipe game in the hub), grid-rotation (GUNGE — the first puzzle), directional-merge (TAZO — the first 2048-style game), spatial-packing (DAIRY — the first Blockdoku-style packer), and path-drawing. Roster goes **7 → 6 → 11** — currently at **10** (GUNGE + HOWLER + TAZO + DAIRY landed; WEAVER pending).
...
Phase 5 palette entries (added Unit 2, unreferenced until each game lands): HOWLER reuses `0` (**now referenced** — `PALMAP.howler=0`); **`6` GUNGE** slime lime (**now referenced** — `PALMAP.gunge=6`), **`7` TAZO** electric holo blue (**now referenced** — `PALMAP.tazo=7`; also the `TAZO_COL` tier ramp derives from this family), **`8` DAIRY** warm caramel (**now referenced** — `PALMAP.dairy=8`), **`9` WEAVER** navy + red-sock. (Palette lookup is now the single module-level `PALMAP`/`gamePal` — §7; the old per-map "menu-icon + card lookups" are consolidated.) `8` shipped with DAIRY; `9` hues stay **tentative** — retune when WEAVER is built. Drawing a surface directly (asphalt-court precedent) is still fine where a full palette isn't warranted.
...
**Order of work**: (1) prune CHAIN ✅ **shipped**, (2) pointer-stream contract extension ✅ **shipped (Unit 2)**, (3) palette-cycle fix + new palettes ✅ **shipped (Unit 2)**, (4) games in the order below — GUNGE ✅ **shipped (5.2)**, HOWLER ✅ **shipped (Unit 4)**, TAZO ✅ **shipped (Unit 5)**, DAIRY ✅ **shipped (Unit 6)**, WEAVER pending. Ship them one at a time — each is independently mergeable and independently prunable.
