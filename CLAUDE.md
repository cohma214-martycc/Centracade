...
- **HOWLER** (`par:1800` PROVISIONAL, PALMAP 0, press/drag/release) — **side-on 2D distance course** (6.1 redesign, D29–D32; supersedes the old pseudo-3D depth-swipe). Swipe sets **launch angle + power**; the howler flies a real parabola left→right and you clear four ground markers **in order** (30·60·90·ULTIMATE, `TARGETS_M`), camera panning right in flight then snapping back to the pad. `release` → `vel` (logical px/ms); `power = clamp(vel/VREF 2.0, 0.35, 1.9)` → launch speed `SPEED_REF 1010`·power px/s; angle = `clamp(atan2(-dy,|dx|), ANG_MIN 0.25, ANG_MAX 1.48)` rad, always thrown rightward; integrate under `GRAVITY 1400`; land when it returns to `GROUND_Y 560`. Clear = landing within the target's band (`BAND_M 9`→`BAND_MIN_M 4.5`, narrows down the course); a miss just **costs time** — **no lives, no fail**; the sole terminal is the ULTIMATE clear, a **win** that fires `onOver` once (2nd D14 exception, D30). **Scoring (D31, higher-is-better):** a bleeding `TIME_BUDGET 3000` at `TIME_RATE 40`/s (fast = high) + per-clear centre bonus `round(CENTER_MAX 60 × closeness)`, all `× (1 + WHISTLE_STEP 0.25 × whistles)`; floored at 0, shown live. **Whistle (D32):** absolute swipe-power band `1.85–2.15` px/ms, reachable at **every** distance via angle (near = steep lob, far ≈ 45°) — **co-tuned** so whistle power at 45° reaches ULTIMATE (`1010²/1400 ≈ 729 ≥ 720px`) and `ANG_MAX` is wide enough that the near-target lob (30M needs ~79–85°) is reachable; higher-variance risk/reward; each whistle *clear* stacks toward a ×2 all-whistle WHISTLER run. **No wind — deterministic** (no seeded content, like STACK; §3). Feedback: aim **arrow** (angle) + **power meter** with the whistle band marked + a landing splash; **no landing predictor** (would trivialise a deterministic arc). HUD: score/best, run timer, 1-of-4 progress, off-screen active-target chevron. Rocket redrawn as a foam football head + long finned tail (purple `#4a1e8a→#8a4ae0` / teal `#2ad8b0`, owner-locked; `drawRocket(x,y,s,ang)` rotates to velocity). **All constants `// PROVISIONAL`; on-device `CALIBRATE` whistle pass still owed (§7). Open flag: no fail state ⇒ a player who never clears ULTIMATE can't advance the daily — reachable targets + unlimited retries mitigate; owner to confirm (§7).**
...
- **HOWLER daily-termination guarantee** (NEW, from the 6.1 build) — HOWLER now has **no fail state** (D30): the only terminal is the ULTIMATE clear. A player who can never clear ULTIMATE would never fire `onOver`, stalling the daily gauntlet on this game. Shipped mitigations: the four targets are fixed and all reachable at ~45°, retries are unlimited, and a bailed daily is already forfeit (attempt consumed at start). **Owner call:** accept as-is, or add a budget-exhaustion fallback (when the time budget hits 0 at ~75s, end the run with score = banked centre bonus × whistle mult) — which would be a small, spec-adjacent change to D30/D31, so flagged not built.
- **HOWLER whistle band** — on-device `CALIBRATE` pass still owed; `VREF`/band are synthetic. Now a **power band hittable at any distance** (angle-chosen range); the co-tune shipped as `SPEED_REF 1010` + `ANG 0.25–1.48` (whistle@45° ≈ 121 M ≥ ULTIMATE; 30M whistle needs a ~79–85° lob) — reconfirm on device. *The old "6.1 easing magnitudes" item is void — the depth-swipe model was replaced, not eased.*
