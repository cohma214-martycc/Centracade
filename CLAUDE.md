...
- **B9 — daily tile copy is stale post-D33.** `drawDailyTile()` still reads `EVERY GAME · ONE SCORE`, but since D33 the daily is 6 of 11. Found during the 7.0 audit and **deliberately not fixed there** — 7.0's acceptance test is byte-identical rendered text, so a copy fix could not ride in that commit. One-line change; it is a hub string, not a per-game one, so it can either stay inline or become a `hub` pack field when 7.1 touches the tile. Owner may prefer different wording (`SIX GAMES · ONE SCORE`, `TODAY'S SIX · ONE SCORE`).
