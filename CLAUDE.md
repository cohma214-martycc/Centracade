...
Phase 5 palette entries (added Unit 2, unreferenced until each game lands): HOWLER reuses `0` (**now referenced** — `PALMAP.howler=0`); **`6` GUNGE** slime lime (**now referenced** — `PALMAP.gunge=6`), **`7` TAZO** electric holo blue (**now referenced** — `PALMAP.tazo=7`; also the `TAZO_COL` tier ramp derives from this family), **`8` DAIRY** warm caramel, **`9` WEAVER** navy + red-sock. (Palette lookup is now the single module-level `PALMAP`/`gamePal` — §7; the old per-map "menu-icon + card lookups" are consolidated.) `8`/`9` hues stay **tentative** — retune when the owning game is built. Drawing a surface directly (asphalt-court precedent) is still fine where a full palette isn't warranted.
...
2. **Recolour the Mega Howler — purple body, green/teal fins.** Cosmetic change to `drawRocket()` **and** the `drawMenuIcon` howler branch (both currently ember/orange body from PALS[0] with `#e85d15` fins). **Reconcile with the palette map**: HOWLER is `PALMAP.howler=0` (ember) in the single `PALMAP` (§7), so the *card/background/HUD glow* is ember while the *rocket* would go purple — decide whether that split is intended (rocket is a distinct object, like GUNGE's drawn surfaces) or whether HOWLER should move to a purple-ish palette entry (note PALS[2] purple is the reserved hub palette — do not repurpose it for a game; add/point at a new entry if a full repalette is wanted). Cheapest path: leave PALMAP at 0, just recolour the drawn rocket sprite + icon.
