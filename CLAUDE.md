...
**Phases 1–5 are SHIPPED.** The roster is **13 games shipped** (STACK · SCAN · HOWLER · SWINGBALL · GUNGE · CHATTER · TAZO · SCRAMBLE · DAIRY · KNUCKLEBONES · WEAVER · **RECALL** · **LOCK**) **targeting 15 gross** (D36), plus the once-a-day seeded **TUCK SHOP RUN** daily, which draws a day-seeded subset rather than the whole roster (D33; `DAILY_PICK` is **5** since RECALL landed, D37). The roster was reopened at D36 for **four** additions — RECALL ✅ · LOCK ✅ · ROUND · CHUTE (Phase 7.5) — after which it closes again pending prune calls (D26). Work now proceeds in this order:
...
- **Prefix-deterministic — RECALL and LOCK (7.5)**, the strongest form in the codebase and worth understanding before copying it. The stream is consumed as **one growing prefix**, at a rate that is a pure function of *how far the player got*, never of *how they played*. Skill changes how far down the sequence you get, never what it is.
  ...
  - ⚠ Both are fragile the same way: a per-round decoy/shuffle (RECALL) or a seeded hint/diary pick (LOCK) would make consumption depend on the player again, which is B1 in a different costume. **Draws per unit must stay a function of the index, not the play.**
...
- **Menu**: page 0 = full-page TUCK SHOP RUN tile; game cards 2×2 from page 1 (`PER_PAGE=4`; `menuPageCount()` = `1+ceil(GAMES.length/4)`, so **13 games ⇒ 5 pages** — LOCK opened page 4 and sits there alone until ROUND and CHUTE land). RECALL before it filled the 11-game roster's last-page gap without moving a card. Card selection resolves on `pointerup` (drag = page swipe); game taps fire on `pointerdown`. Swipe, dots/arrows, ←/→ keys all page. Menu subtitle count derives from `GAMES.length`.
...
2. **§2 keys table** — ✅ `arc_recall_best`, ✅ `arc_lock_best`; add `arc_round_best`, `arc_chute_best` as each ships. No legacy-key inheritance for any of them (D1 precedent).
3. **§3 determinism model** — ✅ RECALL and ✅ LOCK both added to the *prefix-deterministic* bullet (draws per unit are a function of the index, not the play; stronger than fixed-count and documented as such). Add ROUND and CHUTE to the **fixed-count-per-board/round** bullet. None of the four may join the *inherently divergent* list.
4. **§3 pointer-stream note** — ✅ RECALL and ✅ LOCK logged tap-only. ROUND and CHUTE become `press`+`release` consumers (no `drag`), TAZO-style.
5. **§4** — ✅ RECALL, ✅ LOCK. One as-shipped entry per game, written from the code, not from this brief.
6. **`GAMES` array** — ⚠ **pacing is moot, and the brief was wrong about this.** Since D33 the daily is a day-seeded *shuffle* of a subset, so this array no longer paces a run — it paces the **menu**. RECALL was therefore **appended**, not slotted mid-array: at 11 games the last page held 3 cards and a gap, so appending filled the gap and moved no existing card off its page or position. ✅ LOCK was appended the same way (it opens page 4 alone; 13 cards can't avoid a 1-card page). Do the same for ROUND/CHUTE unless there's a menu-layout reason not to.
7. **`PALMAP`** — ✅ `recall:10` on a new `PALS[10]` (dial-up magenta, ~310°, the largest unused hue gap — reuse would have put a 12th card in an already-crowded band next to WEAVER's red). Appending `PALS` is safe by construction (§3). ✅ `lock:4` **reuses** SCRAMBLE's party pink — after RECALL every remaining hue gap sits inside the crowded green band, and reuse is precedented (HOWLER shares `0`, CHATTER shares `3`). Two entries still to add; a missing one must still degrade to `PALS[2]`, never throw.
...
- **Menu paging at 15 cards.** ✅ 5 pages as of LOCK (13 cards); the dots/arrows were screenshot-checked at 5 and read cleanly. **LOCK currently sits alone on page 4** — unavoidable at 13 cards, and it fills to 3 when ROUND and CHUTE land. No further paging work expected before 15.
