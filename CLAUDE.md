...
**Phases 1–5 are SHIPPED.** The roster is **15 games shipped** (STACK · SCAN · HOWLER · SWINGBALL · GUNGE · CHATTER · TAZO · SCRAMBLE · DAIRY · KNUCKLEBONES · WEAVER · **RECALL** · **LOCK** · **ROUND** · **CHUTE**) — **the D36 target of 15 gross is MET and the roster is closed again**, pending prune calls (D26), plus the once-a-day seeded **TUCK SHOP RUN** daily, which draws a day-seeded subset rather than the whole roster (D33; `DAILY_PICK` is **5** since RECALL landed, D37). All four Phase-7.5 additions are in: RECALL ✅ · LOCK ✅ · ROUND ✅ · CHUTE ✅. Work now proceeds in this order:
...
- **Phase 7 — SKINS ⬅ IN PROGRESS.** Cosmetic era packs (default: NZ 90s/00s) over frozen mechanics. **7.0 (the string extraction) is SHIPPED** — display text now lives in `SKINS.nz90`, not in the game objects, and all fifteen games are pack-native. Next: 7.1 selector + unlock, then 7.2's first alternate era pack.
- **Phase 7.5 — ROSTER EXPANSION ✅ COMPLETE (2026-08-02).** All four shipped pack-native, after the 7.0 extraction and before the first alternate era pack (D38). Next in sequence: **7.1 selector + unlock**, then **7.2's first alternate era pack**.
...
| `arc_lock_best` | LOCK best (Phase 7.5 — fresh key, no legacy inheritance, D1 precedent) |
| `arc_round_best` | ROUND best (Phase 7.5 — fresh key, no legacy inheritance, D1 precedent) |
| `arc_chute_best` | CHUTE best (Phase 7.5 — fresh key, no legacy inheritance, D1 precedent) |
...
- `PALS` — **twelve** palettes `{top, bot, ring, acc, glow, sky}`: `0` ember/orange, `1` teal, `2` purple (**hub palette** — menu, interstitials, end card, pause, and the `gamePal` fallback), `3` backyard green, `4` party pink, `5` amber, `6` GUNGE slime lime, `7` TAZO holo blue, `8` DAIRY caramel, `9` WEAVER navy/red-sock, `10` RECALL dial-up magenta (~310° — the largest unused hue gap in the set; picked over reuse so the 12th card reads distinctly on the purple hub). **LOCK reuses `4`** (diary pink): after RECALL the wheel is full — every remaining gap sits inside the crowded green band — and reuse is precedented (HOWLER shares `0`, CHATTER shares `3`). `11` ROUND lawn green (~120°) — added rather than reused because the one gap still wider than 50° (GUNGE's yellow-lime 90° → SWINGBALL's blue-mint 148°) happens to be exactly the hue a mowing game wants. **CHUTE reuses `7`** (TAZO's navy/electric blue) as a pre-dawn depot — at 15 games the wheel is genuinely closed, and the theme wanted a colour that already existed. The four Phase-5 hue sets are still `// PROVISIONAL`. **Appending is safe by construction** — `CHATTER_PALS` is an explicit index list, not a modulo over `PALS.length`, so new rows cannot recolour CHATTER (the only reference to `PALS.length` in the file is that comment).
...
**Pointer stream (D12, wired):** a game declares the stream by defining `press`. The router (`gameGesture` + `gameDown()`) then forwards down/move/up and never calls `tap()` during play; `ready`/`over` interactions still resolve through `tap()` **on release**. Works identically in free play and the gauntlet. `pointercancel` routes to `release(lastCoords)` — an abandoned drag can never hang a gesture. Consumers: HOWLER (press+drag+release, drag = aim arrow + power meter), TAZO, **ROUND** and **CHUTE** (press+release, no drag by design), WEAVER (full consumer — all three carry live state). GUNGE, DAIRY (D13) and **RECALL** + **LOCK** (7.5) are tap-only. Games without `press` keep zero-latency tap-on-pointerdown, untouched — so B6's release-latency cost does not apply to them. Keep `gameGesture` and `menuDrag` strictly separate.
...
- **Prefix-deterministic — RECALL, LOCK, ROUND and CHUTE (7.5)**, the strongest form in the codebase and worth understanding before copying it. The stream is consumed as **one growing prefix**, at a rate that is a pure function of *how far the player got*, never of *how they played*. Skill changes how far down the sequence you get, never what it is.
  - **RECALL**: `init()` draws item 1, each *cleared* round appends exactly one more, a miss/replay consumes **nothing**. Draw count == rounds reached; a round-20 player and a round-8 player share the first 8 items byte-for-byte. Verified headless (a 2-miss run is a byte-exact prefix of a clean run on the same seed).
  - **LOCK**: `newLock()` draws exactly `slots` numbers, and `slots` is a pure function of the lock index (3, then 4 from `ESCALATE_AT`) — so draws-before-lock-N are identical for everyone and lock N's code is shared. **Guesses consume nothing**, and the diary line is picked by lock index, not a draw. Verified headless (a fast player and one who burns two lives get identical codes; totals differ, 50 vs 26 draws).
  - **ROUND**: the WHOLE course is dealt at stage start — exactly **3 draws per row** × `ROWS_BASE + stage·ROWS_INC` rows. A hit costs a life but never regenerates the course, so draws-before-stage-N are a pure function of N and stage N is byte-identical for everyone. Verified headless (a clean run and one that takes hits produce identical courses). ⚠ **Spawn-as-you-scroll is banned here** and the code says so at the generator: hazard count would track the player's pace and the shared stream would drift — that is literally B1.
  - **CHUTE**: the batch's whole item list is pre-generated at stage start — exactly **3 draws per item** (colour, fold, size) × `itemCount(stage)`, and the *criterion* is derived (`stage % 3`), never drawn. Sorting, missing and the telegraph all consume nothing, so batch N is byte-identical for everyone. Verified headless (a perfect sorter and one who lets **every** item fall get identical batches). It deliberately does **not** inherit SCAN's streaming-draw exemption.
  - ⚠ All four are fragile the same way: a per-round decoy/shuffle (RECALL), a seeded hint/diary pick (LOCK), a mid-course spawn (ROUND), or a streamed arrival (CHUTE) would make consumption depend on the player again, which is B1 in a different costume. **Draws per unit must stay a function of the index, not the play.**
...
- **Menu**: page 0 = full-page TUCK SHOP RUN tile; game cards 2×2 from page 1 (`PER_PAGE=4`; `menuPageCount()` = `1+ceil(GAMES.length/4)`, so **15 games ⇒ 5 pages** — LOCK opened page 4, ROUND and CHUTE filled it to three; the roster fits with a slot to spare). RECALL before them filled the 11-game roster's last-page gap without moving a card. Card selection resolves on `pointerup` (drag = page swipe); game taps fire on `pointerdown`. Swipe, dots/arrows, ←/→ keys all page. Menu subtitle count derives from `GAMES.length`.
...
- **LOCK** (`par:600` PROVISIONAL, PALS[4] shared with SCRAMBLE, tap-only) — **Phase 7.5, game 2 of 4; shipped 2026-08-02.** Mastermind against a diary padlock. **3 dials × 6 charms with repeats** (216) for locks 1–2, **4 dials from lock 3** (`ESCALATE_AT`, 1296), capped at 4. Tap a dial to cycle its charm, tap the drawn SUBMIT (GUNGE-valve / DAIRY-rotate-button precedent). Feedback is Mastermind pegs — gold filled = right charm right dial, white ring = right charm wrong dial — computed with **strict multiset counting** (exacts removed first, then each remaining guess symbol consumes at most one remaining code symbol). *That algorithm is the whole game*: the naive version double-counts on repeats and is invisible until one shows up, so it is verified **exhaustively** (all 216 codes × 216 guesses at 3 dials, plus 120k sampled at 4) against an independently-written reference. **Six charms differ in SHAPE first, hue second** (heart · star · flower · moon · bolt · diamond) — six symbols on a dark field is exactly where colour-only coding breaks (the TAZO 6.2c lesson). **Guess budget 5 at both widths** (`BUDGET3`/`BUDGET4` — the brief said 5/6; see the retune note below). Exhausting it costs **−1 life**, reveals the code, and deals a fresh lock; 3 lives = over (endless, D39). The clock is a **BONUS METER only** (D3/D19): it drains, it banks on solve, and hitting 0 never fails the lock or ends the run (verified). **Score on solve** = `guessesLeft × slots × 10 + floor(meterLeft)`; a failed lock banks nothing. Solving reveals one line of **her diary** — the per-lock reward beat — chosen by **lock index, never a seeded draw** (§3). `CLOCK3 32`/`CLOCK4 44`s, `PTS_PER_GUESS 10`, `SOLVED_TIME 2.8`/`FAILED_TIME 2.3`. Theme (big sister's diary padlock, D43) is brand-free and **D15-cleared** (owner, 2026-08-02). All constants `// PROVISIONAL` until D41.
- **ROUND** (`par:800` PROVISIONAL, PALS[11] lawn green, press+release no drag) — **Phase 7.5, game 3 of 4; shipped 2026-08-02.** Three-lane scrolling dodge, themed as mowing the section (D43/D44). Coarse L/R swipe past `DIR_THRESH 30` hops one lane over `HOP_TIME 0.12`s; the hop is **not cancellable mid-flight**, a vertical or sub-threshold swipe is a free no-op, and swiping past the outer lane is a free no-op and **never a life** (§9). Collision uses the lane nearest the mower's *drawn* x, so what you see is what you hit. **Course is dealt WHOLE at stage start** (§3) — `ROWS_BASE 16 + stage·ROWS_INC 2` rows at `ROW_GAP 78`px, scrolling at `BASE_SPEED 150 + stage·SPEED_INC 16` capped at `SPEED_CAP 300` (the cap exists so a row interval, 0.26s at cap, always exceeds a hop). **Solvable by construction (D16):** a safe lane is carved first, walking ±1 per row so it is always reachable in one hop, and hazards only ever go in the other two lanes — a row can never be fully blocked. Verified over 2000 courses: zero blocked rows, zero dead ends under a one-hop-per-row reachability sweep. **Scoring is additive, never multiplicative** (the GUNGE par lesson): `PICKUP 10` per tuft of long grass, `STAGE_BONUS 25 × (stage+1)` on completing a strip, plus `CLEAN_BONUS 40 × (stage+1)` for a strip taken with zero hits. Distance *is* the clock — the HUD bar is course progress, not a timer (D3). Hit = −1 life + `INVULN 0.9`s of flashing so one mistake can't cost two; 3 = over (endless, D39). Hazards are the hose, the sprinkler and the brick in the long grass, art derived from the row index (no draw). A cut stripe trails the mower. Theme is brand-free and **D15-cleared** (owner, 2026-08-02); the display `name` is still the working roster name — the theme rides on tag/ranks/banners, so renaming is one pack string (§7). All constants `// PROVISIONAL` until D41.
- **CHUTE** (`par:700` PROVISIONAL, PALS[7] shared with TAZO, press+release no drag) — **Phase 7.5, game 4 of 4; shipped 2026-08-02. The roster's rule-switching game.** A paper drops from the chute mouth toward a `FALL_Y 404` line; swipe **L / DOWN / R** past `DIR_THRESH 30` to throw it into the bottom-left / centre / right bag, so direction maps to bag *position* and is spatially self-evident. **The descent IS the timer** — no separate bar; the window is `(FALL_Y − MOUTH_Y 196) / (FALL_BASE 80 + stage·FALL_INC 8, capped FALL_CAP 150)`, ≈2.6s at batch 1 down to ≈1.4s. **D42 satisfied structurally:** every item carries three independent attributes (colour · fold · size) and **each bag has a fixed identity across all three** — bag 0 is (newsprint, rolled, thin), bag 1 (orange, flat, medium), bag 2 (cyan, bagged, thick) — all three drawn on every bag at all times. The flip changes only **which attribute is scored** (`crit = stage % 3`, derived, never drawn), highlighted on the bags and announced by a `TELEGRAPH 2.0`s banner **before** it takes effect. No bag ever moves, gets relabelled, or lies; verified over all 81 item/criterion combinations that the scored bag is exactly the bag drawn with that attribute. **Outcomes (owner call, 2026-08-02):** a **wrong bag costs a life and the streak**; an item that falls through **unsorted costs the streak only, never a life** — a life is the price of a committed wrong decision, not of hesitation (§9). 3 lives = over (endless, D39). **Score = `BASE_PTS 10 × mult`, `mult = min(MULT_CAP 4, 1 + floor(streak/STREAK_STEP 6))`** — accuracy-gated throughput, the differentiator against SCAN (raw clear rate) and SCRAMBLE (evenness). Colour categories are newsprint / orange / cyan, never red+green: colour is itself a scored channel here, so it is the one place the TAZO 6.2c lesson cannot be answered with shape. Batch of `min(ITEMS_CAP 18, ITEMS_BASE 10 + stage·ITEMS_INC 2)` items; the next item previews inside the funnel. Theme (the paper run, D43/D44) names no masthead, is brand-free, and is **D15-cleared** (owner, 2026-08-02). All constants `// PROVISIONAL` until D41.
...
#### 6.5 — Roster-wide par retune (needs 6.0 data) — ⏸ DEFERRED by D41 (roster precondition now MET)

**All 15 games exist as of 2026-08-02, so the blocker is now purely the DATA.** Runs once, when telemetry covers all fifteen (D41): par is the exchange rate between games in the daily total, so retuning 11 and re-retuning 15 wastes the data and produces two incomparable eras of totals. The skew notes below stay valid as the inputs to that single pass.
...
All constants below are `// PROVISIONAL` design intent, not spec-final. Each game's **theme** sub-bullet is **✅ D15-CLEARED and shipped** (owner, 2026-08-02) — all four mains are brand-free, naming no ISP, product or masthead. The *banked alternates* under each game are a separate matter and remain parked (D43).
...
#### LOCK — deduction — ✅ SHIPPED (2026-08-02)

**As-shipped truth is the §4 entry.** One spec constant changed at build, on sim evidence, under the "all constants below are `// PROVISIONAL`" licence at the top of this block:

- **`BUDGET4` 6 → 5.** The brief's 5-at-3-dials / 6-at-4-dials produced a game that **does not end**. A naive consistent-guess solver (picks any guess still consistent with the pegs — weaker than a real deducer) solves 4 dials in 6 guesses **99.4%** of the time, so a methodical player almost never loses a life: **median run 247 locks ≈ 77 minutes**, against a 5–10 min daily target. At 5 guesses it solves 85.5% → **median 20 locks / p90 37 ≈ 6–11 min**, which is the band. It also fixed an **inversion**: at 6 guesses the "escalation" to 4 dials was *easier* than 3 dials (0.6% vs 8.5% failure), the opposite of the brief's stated intent that escalation is where run-length pressure lives. At 5 it is 14.1% vs 8.7% — escalation now bites. This is the D27/D28 pattern (a faithful sim moving a provisional constant); **owner may re-call it** (§7).
- Everything else shipped exactly as specced.
...
- **Theme — main: big sister's diary padlock. ✅ D15-CLEARED + SHIPPED (owner, 2026-08-02).** Cracking the code reveals one line of the diary on the solve screen — a reward beat per lock, which suits LOCK's chained structure better than a bare SOLVED banner. Three failed locks and she comes back into the room (the existing 3-lives terminal, reskinned). Diary lines are pack strings; they need writing as part of the pack and are subject to the same tone bar as the rest of the copy. *Banked alternate (7.2):* **the Sky decoder parental lock** — four digits between you and something you're not meant to be watching; the D19-style bonus meter reads as how long until someone comes down the hall. **Named brand — D15 clearance required.** (The bike-shed combination was the third option and was not selected; leaving it unlogged rather than half-parked.)

#### ROUND — lane dodge — ✅ SHIPPED (2026-08-02)

**As-shipped truth is the §4 entry.** Built to spec; no constant was re-called. Two notes for CHUTE, which inherits the same swipe primitive:
- The "generated WHOLE at stage start" rule did the work it was written for — the generator carves a safe lane *first* and hangs hazards off it, so solvability is structural rather than checked. CHUTE's pre-generated item list should follow the same shape.
- `hitHazard()` got a one-shot `st!=='play'` guard. Without it a life-losing call after death re-fires `onOver`, which in the daily would advance two games at once. Worth copying.
...
- **Theme — main: mowing the lawns. ✅ D15-CLEARED + SHIPPED (owner, 2026-08-02).** Three strips up the section map onto the three lanes. Pickups are the rows still uncut; hazards are the hose, the sprinkler, and the brick left in the long grass. Brand-free as shipped. *Banked alternate (7.2) — STILL PARKED:* **school cross country** — three lanes of a borrowed farm paddock, dodging cow pats, an electric fence, and the kid who sprints the first hundred metres then walks. Brand-free, but era-agnostic to the point of being timeless, which makes it a weaker nostalgia slot than the lawns.

#### CHUTE — sort under a shifting rule — ✅ SHIPPED (2026-08-02)

**As-shipped truth is the §4 entry.** Built to spec; the one open question was answered by the owner at build:
- **An item that falls through unsorted costs the STREAK only, never a life** (owner, 2026-08-02) — the recommendation on file. A wrong bag costs a life *and* the streak.
- D42 is satisfied *structurally* rather than by discipline: bags carry a fixed identity across all three attributes and always draw all three, so "the mapping never changes and never lies" is a property of the data, not a rule someone has to remember. Verified exhaustively over all 81 item/criterion combinations.
...
- **Theme — main: the paper run (depot sort). ✅ D15-CLEARED + SHIPPED (owner, 2026-08-02).** Three bags — the daily, the free community paper, the junk mail. The D42 criterion flip runs publication → street run → houses with a No Junk Mail sticker, telegraphed as always; bin mapping and labels never change. Cleared on the basis that the publications stay generic — the copy reads `SORT BY MASTHEAD / FOLD / SIZE` and names no title. **Naming a specific masthead would need a fresh D15 pass.** *Banked alternate (7.2):* **the Video Ezy return chute** — new releases, weeklies, overnights, with the criterion flipping to genre, and an unrewound tape costing you. **Named brand — D15 clearance required.**

---

#### 7.5 build-time checklist (rewrite these ONLY in the commit that ships each game)

1. **Header roster line** — ✅ 12 at RECALL, ✅ 13 at LOCK; → 14/15 as each lands.
2. **§2 keys table** — ✅ all four (`arc_recall_best`, `arc_lock_best`, `arc_round_best`, `arc_chute_best`). No legacy-key inheritance for any of them (D1 precedent).
3. **§3 determinism model** — ✅ RECALL and ✅ LOCK both added to the *prefix-deterministic* bullet (draws per unit are a function of the index, not the play; stronger than fixed-count and documented as such). ✅ ROUND and ✅ CHUTE joined them (whole course/batch dealt per stage, 3 draws per row/item). ✅ None of the four joined the *inherently divergent* list.
4. **§3 pointer-stream note** — ✅ RECALL and ✅ LOCK logged tap-only; ✅ ROUND and ✅ CHUTE logged as `press`+`release` consumers (no `drag`), TAZO-style.
5. **§4** — ✅ all four (RECALL, LOCK, ROUND, CHUTE). One as-shipped entry per game, written from the code, not from this brief.
6. **`GAMES` array** — ⚠ **pacing is moot, and the brief was wrong about this.** Since D33 the daily is a day-seeded *shuffle* of a subset, so this array no longer paces a run — it paces the **menu**. RECALL was therefore **appended**, not slotted mid-array: at 11 games the last page held 3 cards and a gap, so appending filled the gap and moved no existing card off its page or position. ✅ LOCK, ✅ ROUND and ✅ CHUTE were appended the same way and share page 4. No existing card ever moved.
7. **`PALMAP`** — ✅ `recall:10` on a new `PALS[10]` (dial-up magenta, ~310°, the largest unused hue gap — reuse would have put a 12th card in an already-crowded band next to WEAVER's red). Appending `PALS` is safe by construction (§3). ✅ `lock:4` **reuses** SCRAMBLE's party pink — after RECALL every remaining hue gap sits inside the crowded green band, and reuse is precedented (HOWLER shares `0`, CHATTER shares `3`). `11` ROUND lawn green (~120°) — added rather than reused because the one gap still wider than 50° (GUNGE's yellow-lime 90° → SWINGBALL's blue-mint 148°) happens to be exactly the hue a mowing game wants. **CHUTE reuses `7`** (TAZO's navy/electric blue) as a pre-dawn depot — at 15 games the wheel is genuinely closed, and the theme wanted a colour that already existed. ✅ `round:11` on a **new** `PALS[11]` lawn green — the last gap wider than 50° sat exactly where a mowing game wants to be, so this one earned a row where LOCK didn't. ✅ `chute:7` **reuses** TAZO's navy as a pre-dawn depot. Rule that emerged: *add a palette when the free hue gap IS the theme, reuse when an existing hue already fits it.* A missing entry must still degrade to `PALS[2]`, never throw.
...
9. **§6.5 par retune** — the **roster precondition is now met** (all 15 exist). Still blocked on the *data*: D41 needs telemetry from all fifteen before the single pass runs.
...
| D40 | **The ≤1-of-{TAZO, DAIRY} daily constraint is unchanged.** LOCK is a §7 run-length **watch item**, not a constraint member — its fixed guess budget hard-bounds each lock and the life count bounds the run. Revisit only from telemetry. *(⚠ The stated premise was measurably false at the brief's 5/6 guess budgets — a consistent solver almost never lost a life, giving a ~77 min median run. LOCK shipped at 5/5, which restores the premise: median ~20 locks / 6–11 min. The **decision** — LOCK is not a constraint member — stands; see §7.)* |
...
| D43 | **Each Phase-7.5 game ships with one main `nz90` theme and one banked alternate** (owner-directed, 2026-08-02). Main: RECALL = dial-up modem handshake · LOCK = big sister's diary padlock · ROUND = mowing the lawns · CHUTE = the paper run (depot sort). Banked alternates for the 7.2 pack: RECALL = delivery jingle · LOCK = Sky decoder parental lock · ROUND = school cross country · CHUTE = Video Ezy return chute. **✅ ALL FOUR MAINS ARE D15-CLEARED (owner, 2026-08-02)** — dial-up, diary padlock, mowing the lawns and the paper run, each including its receipt line (`📞`/`🔒`/`🌱`/`📰`). Every one shipped brand-free: no ISP, product or masthead is named anywhere in the pack. **All four banked alternates remain PARKED** — they carry named brands (Eagle Boys / Pizza Hut, Sky, Video Ezy) or an uncleared reference (school cross country), and each needs its own D15 pass before 7.2 can ship. |
...
- **`shareLabel`/`shareIcon` for the ORIGINAL ELEVEN (D34).** Proposed `nz90` set below; owner clears each snack reference (D15). The four Phase-7.5 rows are already cleared — these eleven are not.

*Phase 7.5 (roster expansion):*

- **ROUND continuous-control gap.** Lane-snap shipped as the owner picked it, so the roster still has **no continuous-input game** at 14. Unchanged by the build; revisit at prune time or if a 16th slot ever opens.
- **ROUND display name.** Shipped as `ROUND`, the working roster name, which doesn't say "mowing" the way `tag`/ranks/banners do. It's a single pack string (`SKINS.nz90.games.round.name`) — rename it freely if you'd rather the card read e.g. `THE LAWNS`.
- **ROUND par + difficulty.** `par:800` ≈ reaching strip 5. Untested against a human — the safe lane is always there to be found, so the open question is whether a good player ever *dies* or just grinds strips at rising speed until the speed cap. `SPEED_CAP 300` bounds it, but the cap also bounds difficulty; if runs go long, the lever is `ROWS_INC`/`SPEED_INC`, or letting hazards occupy the safe lane once per course at high stages. Measure at D41.
- ~~**CHUTE unsorted-item rule**~~ — RESOLVED 2026-08-02 (owner): **unsorted resets the STREAK only, never a life**; only a wrong bag costs a life (and the streak too). Shipped and verified.
- **CHUTE par + difficulty.** `par:700` is a guess: a competent player scoring ~10×1.4 average over ~40 sorted items. Untested against a human — the open question is whether the criterion flip is disorienting enough to cost lives, or whether a careful player simply never picks a wrong bag and only ever loses streak. If runs go long, the levers are `FALL_INC`/`FALL_CAP` (a tighter window), never a time cap (D3). Measure at D41.
- **LOCK guess budget — owner may re-call.** D40's premise ("its fixed guess budget hard-bounds each lock and the life count bounds the run") turned out **measurably false at the brief's 5/6 budgets**: a naive consistent solver clears 4-dial locks 99.4% of the time, giving a median run of **247 locks ≈ 77 min**. Shipped at **5/5** instead → median 20 locks / p90 37 ≈ **6–11 min**, and escalation now raises difficulty (14.1% vs 8.7% failure) instead of lowering it. Sim only — a real human is worse than a consistent solver, so expect *shorter* runs than this. Confirm against telemetry at D41; the lever if it still runs long is `BUDGET3` 5 → 4 (median drops to ~15 locks), not a time cap (D3).
- **LOCK run length after the budget fix.** Still the D40 watch item, just a much smaller one. Measure at first telemetry; escalate to the ≤1-per-day constraint set only if data demands it.
...
- ~~**D15 clearance for the Phase-7.5 mains (D43)**~~ — RESOLVED 2026-08-02: **all four mains are cleared** (dial-up · diary padlock · mowing the lawns · the paper run), each with its receipt line. Every one is brand-free — no ISP, product or masthead is named in the shipped pack. **D43's mains are closed.**
- **D15 clearance for the four banked ALTERNATES — still owed, and it blocks 7.2.** Each carries a reference the owner has not cleared: Eagle Boys and the Pizza Hut `0800 83 83 83` number (RECALL alternate), Sky (LOCK alternate), Video Ezy (CHUTE alternate), and school cross country (ROUND alternate — brand-free but unconfirmed). None may ship in the first alternate era pack until cleared individually. A named masthead would also pull CHUTE's paper run back into D15.
- ~~**Share icons and share labels for the four new games**~~ (D34 receipt lines) — RESOLVED 2026-08-02: `📞` dial-up · `🔒` diary lock · `🌱` the lawns · `📰` paper run, all D15-cleared with their themes. ⚠ The **original eleven** rows in the D34 table below are a separate matter and remain owner-uncleared.
...
- ~~**LOCK diary lines**~~ — RESOLVED at build: **8 lines in the pack, cycled by lock index** (never a seeded draw — that would break §3). Written to the era's tone and deliberately innocuous, ending on `IF YOU ARE READING THIS: GET OUT.` as the payoff. Owner may rewrite them freely; they are pure pack strings and touch no number.
- ~~**Menu paging at 15 cards**~~ — RESOLVED: 5 pages, with LOCK/ROUND/CHUTE on page 4 and one slot spare. Dots and arrows screenshot-checked at 5 pages and read cleanly.
- ~~**Byte budget**~~ — MEASURED at CHUTE (2026-08-02): `index.html` is **286 KB raw / 84 KB gzipped**, up from 226 KB raw across the four games. Comfortably inside anything iOS Safari struggles with, and the wire cost is what matters. No action; re-check if the roster ever grows again.
...
`nz90` mapping (emoji are the text-share icons). The first eleven rows are **proposed, owner-uncleared**; the four Phase-7.5 rows are **cleared and shipped**:
...
| **LOCK** | **🔒** | **diary lock** | — ✅ **D15-cleared + shipped** (2026-08-02) |
| **ROUND** | **🌱** | **the lawns** | — ✅ **D15-cleared + shipped** (2026-08-02) |
| **CHUTE** | **📰** | **paper run** | — ✅ **D15-cleared + shipped** (2026-08-02) |
...
These live in the Phase 7.0 pack as `SKINS.nz90.games.<id>.shareLabel` / `shareIcon` (the extraction shipped 2026-08-01; the interim `SHARE_NZ90` object is gone). **The four Phase-7.5 rows are D15-cleared** (2026-08-02); the **original eleven above them are still owner-uncleared** — they ship as working values until each snack reference is cleared.
